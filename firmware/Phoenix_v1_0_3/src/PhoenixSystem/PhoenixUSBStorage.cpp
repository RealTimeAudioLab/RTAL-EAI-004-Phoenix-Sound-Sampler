...
bool PhoenixUSBStorage::begin(Mode mode) {
  ...
  Serial.printf("C028e USB MSC %s sectors=%lu sectorSize=%u bootSig=%02X%02X\n",
...
bool PhoenixUSBStorage::endAndRemount() {
  ...
  _remountOk = SD.begin(PHX_SD_CS, SPI);
  Serial.printf("C028e USB MSC exit remount=%s writes=%s readKB=%lu writeKB=%lu errors=%lu\n",
                _remountOk ? "OK" : "FAIL",
                hadWrites ? "YES" : "NO",
...
void PhoenixUSBStorage::notifyHostStartStop(bool start, bool loadEject) {
  Serial.printf("C028e USB MSC STARTSTOP start=%u loadEject=%u\n",
