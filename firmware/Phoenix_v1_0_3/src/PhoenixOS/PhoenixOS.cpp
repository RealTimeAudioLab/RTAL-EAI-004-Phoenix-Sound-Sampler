...
static bool phxRefreshBankCache() {
  memset(phxBankUsedCache, 0, sizeof(phxBankUsedCache));
  phxBankCacheValid = false;

  if (!phxEnsureSD()) {
    PHX_ERROR_PRINTLN("V1.0.3 BANK CACHE no SD");
    return false;
  }
  ...
  if (!root || !root.isDirectory()) {
    if (root) root.close();
    PHX_ERROR_PRINTLN("V1.0.3 BANK CACHE open failed");
    return false;
  }
  ...
  while (entry) {
    if (entry.isDirectory()) {
      ...
      if (name && strlen(name) == 6U &&
        ...
        if (bank >= 1U && bank <= 99U) {
          char dir[40];
          snprintf(dir, sizeof(dir), "/PHOENIX/BANKS/BANK%02u", (unsigned)bank);
          const bool used = phxBankExists(dir);
  ...
  PHX_INFO_PRINTF("V1.0.3 BANK CACHE ready used=%u\n", (unsigned)found);
  return true;
}
...
void PhoenixOS::begin(bool serviceMode) {
  ...
  // v1.0.3: build bank cache before normal playback/session restore.
  phxRefreshBankCache();
...
void PhoenixOS::update() {
  ...
  _screens.update(_inputState);
  {
    ...
    if (usbAction == 0 || usbAction == 2) {
      ...
      if (ok && changed) phxRefreshBankCache();
