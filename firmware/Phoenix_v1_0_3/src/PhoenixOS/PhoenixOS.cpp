...
static bool phxRefreshBankCache() {
  ...
  if (!phxEnsureSD()) {
    Serial.println("V1.0.3 BANK CACHE no SD");
    return false;
  }
  ...
  if (!root || !root.isDirectory()) {
    if (root) root.close();
    Serial.println("V1.0.3 BANK CACHE open failed");
    return false;
  }
  ...
  Serial.printf("V1.0.3 BANK CACHE ready used=%u\n", (unsigned)found);
  return true;
}
  ...
  if (diskAction >= 0) {
    ...
    if (!phxEnsureSD()) {
      ...
      if (!phxBankExists(bankDir)) {
        ...
        Serial.printf("V1.0.3 LOAD PHASE=AUDIO begin bank=%02u\n", (unsigned)currentBank);
        bool ok = _audio.loadBankFromSD(bankDir);
        Serial.printf("V1.0.3 LOAD PHASE=AUDIO %s\n", ok ? "OK" : "FAIL");
        _audio.setProgressCallback(nullptr, nullptr);
        vTaskDelay(pdMS_TO_TICKS(2));
        if (ok) { Serial.println("V1.0.3 LOAD PHASE=PATTERNS begin"); ok = _screens.loadSequencerConfig(bankDir); Serial.printf("V1.0.3 LOAD PHASE=PATTERNS %s\n",ok?"OK":"FAIL"); vTaskDelay(pdMS_TO_TICKS(2)); }
        if (ok) { Serial.println("V1.0.3 LOAD PHASE=SONG begin"); ok = _songManager.loadConfig(bankDir); Serial.printf("V1.0.3 LOAD PHASE=SONG %s\n",ok?"OK":"FAIL"); vTaskDelay(pdMS_TO_TICKS(2)); }
