...
void PhoenixGUI::drawBoot() {
  drawBootStatus("STARTING", 0);
}

void PhoenixGUI::drawBootStatus(const char *status, uint8_t activity) {
  clear();
  frame(0, 0, 128, 64);
  centered(0, 10, 128, "PHOENIX", false);
  centered(0, 23, 128, "Phoenix Sound Sampler", false);
  centered(0, 36, 128, status && status[0] ? status : "STARTING", false);

  // v1.0.5: small activity bar so long SD/cache operations visibly progress.
  frame(14, 44, 100, 7);
  uint8_t fillW = activity;
  if (fillW > 100U) fillW = 100U;
  const uint8_t innerW = (uint8_t)(((uint16_t)fillW * 98U) / 100U);
  if (innerW > 0U) fill(15, 45, innerW, 5, true);

  centered(0, 56, 128, "v1.0.12a", false);
  send();
}
