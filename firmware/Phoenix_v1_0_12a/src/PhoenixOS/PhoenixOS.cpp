...
// v1.0.12a robust last-bank persistence. NVS remains the primary source,
// while a tiny SD fallback protects against an invalid/missing Preferences
// record. Only the authoritative ACTIVE bank is written here; the Disk
// Utilities cursor must never affect startup restore.
...
void PhoenixOS::restoreLastSession() {
  ...
  Preferences p;
  nvsOpen = p.begin("phoenix", true);
  if (nvsOpen) {
    nvsValid = p.getBool("valid", false);
    nvsBank = p.getUChar("bank", 0);
    screen = p.getUChar("screen", 5);
    slot = p.getUChar("slot", 0);
    p.end();
  }
  if (nvsBank < 1 || nvsBank > 99) nvsValid = false;

  uint8_t fallbackBank = 0;
  const bool fallbackValid = phxReadLastBankFallback(fallbackBank);
  uint8_t bank = nvsValid ? nvsBank : fallbackBank;
  const bool haveSessionBank = nvsValid || fallbackValid;
  ...
  // If NVS pointed at a bank that no longer exists, try the independent SD
  // fallback before giving up. This also repairs NVS after a successful
  // fallback restore.
  if (!loaded && nvsValid && fallbackValid && fallbackBank != nvsBank) {
    char msg[24];
  ...
  if (!loaded) {
    ...
    // Successful restore becomes authoritative in both stores. This repairs
    // stale/corrupt NVS or an out-of-date SD fallback automatically.
    saveSessionNow();
  ...
  _sessionBankCache = _screens.selectedBank();
  _sessionScreenCache = _screens.sessionScreen();
  _sessionSlotCache = _screens.sessionSlot();
}

void PhoenixOS::saveSessionNow() {
  const uint8_t bank = _screens.activeBank();
  const uint8_t screen = _screens.sessionScreen();
  const uint8_t slot = _screens.sessionSlot();

  bool verified = false;
  Preferences p;
  if (p.begin("phoenix", false)) {
    const size_t wValid = p.putBool("valid", true);
    const size_t wBank = p.putUChar("bank", bank);
    const size_t wScreen = p.putUChar("screen", screen);
    const size_t wSlot = p.putUChar("slot", slot);
    p.end();

    // Read back immediately. Preferences writes are tiny and this path runs
    // only for an actual session change / explicit bank load-save.
    Preferences verify;
    if (wValid && wBank && wScreen && wSlot && verify.begin("phoenix", true)) {
      verified = verify.getBool("valid", false) &&
                 verify.getUChar("bank", 0) == bank &&
                 verify.getUChar("screen", 255) == screen &&
                 verify.getUChar("slot", 255) == slot;
      verify.end();
    }
  }

  if (!verified) {
    PHX_INFO_PRINTF("V1.0.12a SESSION NVS VERIFY FAIL bank=%u screen=%u slot=%u\n",
                    (unsigned)bank, (unsigned)screen, (unsigned)slot);
  }
...
void PhoenixOS::saveSessionIfChanged() {
  ...
  const uint8_t bank=_screens.activeBank(), screen=_screens.sessionScreen(), slot=_screens.sessionSlot();
  if(bank==_sessionBankCache && screen==_sessionScreenCache && slot==_sessionSlotCache)return;
  const bool bankChanged = (bank != _sessionBankCache);
  saveSessionNow();
  if (bankChanged) phxWriteLastBankFallback(bank);
}
...
void PhoenixOS::update() {
  ...
  _screens.update(_inputState);
  {
    ...
    if (usbAction == 0 || usbAction == 2) {
      ...
      if (ok && changed) { phxRefreshBankCache(); _screens.invalidateSampleBrowserCache(); }
  ...
  if (diskAction >= 0) {
    ...
    if (!phxEnsureSD()) {
      ...
      SD.mkdir(bankDir);
      delay(1);
      PHX_INFO_PRINTF("V1.0 SAVE PHASE=AUDIO begin bank=%02u\n", (unsigned)currentBank);
      bool ok = _audio.saveBankToSD(bankDir);
      PHX_INFO_PRINTF("V1.0 SAVE PHASE=AUDIO %s\n", ok ? "OK" : "FAIL");
      delay(1);
      if (ok) {
        PHX_INFO_PRINTLN("V1.0 SAVE PHASE=PATTERNS begin");
        ok = _screens.saveSequencerConfig(bankDir);
        PHX_INFO_PRINTF("V1.0 SAVE PHASE=PATTERNS %s\n", ok ? "OK" : "FAIL");
        delay(1);
      }
      if (ok) {
        syncScreensToSong();
        PHX_INFO_PRINTLN("V1.0 SAVE PHASE=SONG begin");
        ok = _songManager.saveConfig(bankDir);
        PHX_INFO_PRINTF("V1.0 SAVE PHASE=SONG %s\n", ok ? "OK" : "FAIL");
        delay(1);
      }
      ...
      if (!phxBankExists(bankDir)) {
        ...
        _audio.setProgressCallback(nullptr, nullptr);
        vTaskDelay(pdMS_TO_TICKS(2));
        if (ok) { PHX_INFO_PRINTLN("V1.0.3 LOAD PHASE=PATTERNS begin"); ok = _screens.loadSequencerConfig(bankDir); PHX_INFO_PRINTF("V1.0.3 LOAD PHASE=PATTERNS %s\n",ok?"OK":"FAIL"); vTaskDelay(pdMS_TO_TICKS(2)); }
        if (ok) { PHX_INFO_PRINTLN("V1.0.3 LOAD PHASE=SONG begin"); ok = _songManager.loadConfig(bankDir); PHX_INFO_PRINTF("V1.0.3 LOAD PHASE=SONG %s\n",ok?"OK":"FAIL"); vTaskDelay(pdMS_TO_TICKS(2)); }
  ...
  // v1.0.11: persist changed bank/screen/slot outside realtime paths.
  // The helper is throttled to at most one NVS write check per second and
  // writes only when the session tuple actually changed.
