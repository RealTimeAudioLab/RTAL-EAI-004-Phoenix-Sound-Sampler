...
bool PhoenixAudioManager::allocateKeygroupMetadata() {
  ...
  Serial.printf("MEM: multisample metadata %u bytes in %s\n",
...
bool PhoenixAudioManager::loadMultisampleConfig(const char *dir){
  if(!dir||!dir[0]) return false;
  // C030 recovery for standalone multisample updates.
  char msTarget[160],msTemp[160],msBackup[160];
  snprintf(msTarget,sizeof(msTarget),"%s/MULTISAMPLE.CFG",dir);
  snprintf(msTemp,sizeof(msTemp),"%s/MULTISAMPLE.MS0TMP",dir);
  snprintf(msBackup,sizeof(msBackup),"%s/MULTISAMPLE.MS0BAK",dir);
  if(SD.exists(msBackup)){if(!SD.exists(msTarget))SD.rename(msBackup,msTarget);else SD.remove(msBackup);} SD.remove(msTemp);

  // C035: validate file/version before destroying the currently loaded map.
  File probe=SD.open(msTarget,FILE_READ); if(!probe) return false;
  int cfgVersion=0; char probeLine[196];
  while(probe.available()){
    size_t n=probe.readBytesUntil('\n',probeLine,sizeof(probeLine)-1); probeLine[n]=0; phxTrimLine(probeLine);
    if(!strncmp(probeLine,"VERSION=",8)){cfgVersion=atoi(probeLine+8);break;}
    if(probeLine[0]=='[') break;
  }
  probe.close();
  if(cfgVersion>PHX_MULTISAMPLE_FORMAT_VERSION){Serial.printf("C035 MULTI reject future version=%d supported=%u\n",cfgVersion,(unsigned)PHX_MULTISAMPLE_FORMAT_VERSION);return false;}

  File f=SD.open(msTarget,FILE_READ); if(!f) return false;
  for(uint8_t s=0;s<SLOT_COUNT;++s) clearMultisample(s);
  int cs=-1,cg=-1,cl=-1; char line[196]; bool importError=false;
  while(f.available()){
    size_t n=f.readBytesUntil('\n',line,sizeof(line)-1);line[n]=0;phxTrimLine(line);if(!line[0]||line[0]=='#')continue;
    if(line[0]=='['){unsigned a=0,b=0,c=0;if(sscanf(line,"[SLOT%u_GROUP%u_LAYER%u]",&a,&b,&c)==3&&a>=1&&a<=4&&b>=1&&b<=16&&c>=1&&c<=3){cs=a-1;cg=b-1;cl=c-1;}else if(sscanf(line,"[SLOT%u_GROUP%u]",&a,&b)==2&&a>=1&&a<=4&&b>=1&&b<=16){cs=a-1;cg=b-1;cl=-1;}else{cs=cg=cl=-1;}continue;}
    ...
    if(cl<0){
      ...
      else if(!strcmp(key,"LOW"))kg.lowNote=constrain(atoi(val),0,127);else if(!strcmp(key,"HIGH"))kg.highNote=constrain(atoi(val),0,127);else if(!strcmp(key,"ROOT"))kg.rootNote=constrain(atoi(val),0,127);else if(!strcmp(key,"TRANSPOSE"))kg.transposeSemitone=(int8_t)constrain(atoi(val),-24,24);else if(!strcmp(key,"FINE_CENT"))kg.fineCent=(int16_t)constrain(atoi(val),-100,100);else if(!strcmp(key,"KEYTRACK_PCT"))kg.keytrackPct=(uint8_t)constrain(atoi(val),0,100);else if(!strcmp(key,"CHOKE"))kg.chokeGroup=constrain(atoi(val),0,8);else if(!strcmp(key,"ONE_SHOT"))kg.oneShot=atoi(val)!=0;else if(!strcmp(key,"CHOKE_FADE_MS"))kg.chokeFadeMs=constrain(atoi(val),0,250);else if(!strcmp(key,"PLAY_MODE"))kg.playMode=constrain(atoi(val),0,2);else if(!strcmp(key,"EXCLUSIVE_GROUP"))kg.exclusiveGroup=constrain(atoi(val),0,8);else if(!strcmp(key,"RETRIGGER"))kg.retriggerLegato=atoi(val)!=0;else if(!strcmp(key,"START_PCT"))kg.startPct=constrain(atoi(val),0,99);else if(!strcmp(key,"END_PCT"))kg.endPct=constrain(atoi(val),1,100);else if(!strcmp(key,"REVERSE"))kg.reversePlayback=atoi(val)!=0;else if(!strcmp(key,"LEVEL")){kg.layer[0].level=constrain(atoi(val),0,100);updateVelocityLayerPan(kg.layer[0]);}else if(!strcmp(key,"PAN")){kg.layer[0].pan=constrain(atoi(val),-100,100);updateVelocityLayerPan(kg.layer[0]);}else if(!strcmp(key,"ENABLED"))kg.enabled=atoi(val)!=0;
    }else{
      ...
      else if(!strcmp(key,"RR_MODE"))vl.rrMode=constrain(atoi(val),0,5);else if(!strcmp(key,"LOOP_MODE"))vl.loopMode=constrain(atoi(val),0,2);else if(!strcmp(key,"LOOP_START_PCT"))vl.loopStartPct=constrain(atoi(val),0,98);else if(!strcmp(key,"LOOP_END_PCT"))vl.loopEndPct=constrain(atoi(val),1,100);else if(!strcmp(key,"LOOP_XFADE_MS"))vl.loopXfadeMs=constrain(atoi(val),0,50);else if(!strcmp(key,"VEL_TO_LEVEL"))vl.velocityToLevelPct=constrain(atoi(val),0,100);else if(!strcmp(key,"VEL_TO_FILTER"))vl.velocityToFilterPct=constrain(atoi(val),0,100);else if(!strcmp(key,"VEL_LOW"))vl.velocityLow=constrain(atoi(val),1,127);else if(!strcmp(key,"VEL_HIGH"))vl.velocityHigh=constrain(atoi(val),0,127);else if(!strcmp(key,"LEVEL"))vl.level=constrain(atoi(val),0,100);else if(!strcmp(key,"PAN"))vl.pan=constrain(atoi(val),-100,100);else if(!strcmp(key,"ENABLED"))vl.enabled=atoi(val)!=0;updateVelocityLayerPan(vl);kg.enabled=true;
    }
  }
  f.close();
  uint32_t fixes=0;for(uint8_t s=0;s<SLOT_COUNT;++s)fixes+=sanitizeMultisampleSlot(s);
  Serial.printf("C035 MULTI load version=%d sanitize=%lu import_error=%u groups=%u/%u/%u/%u\n",cfgVersion,(unsigned long)fixes,importError?1:0,keygroupCount(0),keygroupCount(1),keygroupCount(2),keygroupCount(3));
  return !importError;
}
...
bool PhoenixAudioManager::saveBankToSD(const char *dir) {
  ...
  phxC030CleanupArtifacts(dir);
  Serial.printf("C030 STORAGE COMMIT dir=%s oldmask=0x%02X newmask=0x%02X\n", dir,
                (unsigned)oldMask,
