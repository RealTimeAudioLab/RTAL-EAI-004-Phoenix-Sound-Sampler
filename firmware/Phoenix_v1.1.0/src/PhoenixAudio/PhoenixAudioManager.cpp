...
// v1.1.0 FINAL: Reverb micro-timing is diagnostic-only and is compiled out by default.
// Set to 1 only for targeted measurements. When enabled, exactly one Reverb
// call per 128-frame block is timed instead of every audio frame.
#ifndef PHX_REVERB_TIMING_ENABLED
#define PHX_REVERB_TIMING_ENABLED 0
#endif
...
: _bclkPin(bclkPin), _lrckPin(lrckPin), _doutPin(doutPin), _dinPin(dinPin),
  ...
  _peakTraceSeq(0), _peakTracePeakUs(0), _peakTraceFrame(0), _peakTraceVoices(0), _peakTraceReleasing(0),
  _peakTraceMultisample(0), _peakTraceFilterActive(0), _peakTraceVintageActive(0),
  _peakTraceSlotMask(0), _peakTraceEchoMix(0), _peakTraceReverbMix(0), _peakTraceReverbSize(0),
  _peakTraceReverbDecay(0), _peakTraceReverbDamp(0), _peakTraceState(0), _peakTraceQuattroMode(0),
  _peakTraceTailActive(0), _peakTracePrintedSeq(0),
  ...
  _echoBuffer(nullptr), _echoCapacityFrames(0), _echoDelayMs(250), _echoFeedback(35), _echoMix(20),
  _reverbSize(55), _reverbDecay(55), _reverbDamp(45), _reverbMix(20), _reverbProcessUs(0), _reverbMem(nullptr),
  _reverbCacheReady(false), _reverbCachedSize(255), _reverbCachedDecay(255), _reverbCachedDamp(255),
  _reverbCachedFbQ15(18022), _reverbCachedLpShift(2), _echoWritePos(0),
...
bool PhoenixAudioManager::allocateEchoBuffer() {
  // v1.1.0 DEV2c: mono classic echo, up to 2000 ms at 32 kHz.
  // INIT_SOUND already allows 2000 ms; keep the DSP/UI/storage range consistent.
  const uint32_t frames = 64000UL;
  const size_t bytes = (size_t)frames * sizeof(int16_t);
...
// v1.1.0 FINAL: compact integer stereo Schroeder/Moorer reverb.
// O3 + fast-math are intentionally enabled for this translation unit.
// Four damped feedback combs feed decorrelated stereo allpasses. No pow/sin/sqrt/tanh
// is executed in the render path. The complete delay memory is only 6930 int16 samples
// = 13860 bytes (~13.5 KiB), so DEV3a deliberately keeps it in internal 8-bit RAM to
// remove PSRAM/cache latency from the hottest Reverb delay-line accesses. A fallback
// preserves function if internal allocation should ever fail.
bool PhoenixAudioManager::allocateReverbBuffers() {
  if (_reverbMem) return true;
  static const uint32_t total = 1117U + 1277U + 1423U + 1559U + 337U + 421U + 353U + 443U;
  const size_t bytes = (size_t)total * sizeof(int16_t);

  const uint32_t caps = MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT;
  const size_t internalBefore = heap_caps_get_free_size(caps);
  _reverbMem = (int16_t*)heap_caps_malloc(bytes, caps);
  if (_reverbMem) {
    const size_t internalAfter = heap_caps_get_free_size(caps);
    PHX_INFO_PRINTF("PHX MEM reverb=%u bytes INTERNAL free_before=%u free_after=%u\n",
                    (unsigned)bytes, (unsigned)internalBefore, (unsigned)internalAfter);
  } else {
    _reverbMem = psramFound() ? (int16_t*)ps_malloc(bytes) : (int16_t*)malloc(bytes);
    PHX_INFO_PRINTF("PHX MEM reverb INTERNAL alloc failed; fallback=%s bytes=%u\n",
                    psramFound() ? "PSRAM" : "HEAP", (unsigned)bytes);
  }
  if (!_reverbMem) return false;
  memset(_reverbMem, 0, bytes);
  memset(_reverbCombPos, 0, sizeof(_reverbCombPos));
  memset(_reverbCombLp, 0, sizeof(_reverbCombLp));
  memset(_reverbApPosL, 0, sizeof(_reverbApPosL));
  memset(_reverbApPosR, 0, sizeof(_reverbApPosR));
  _reverbCacheReady = false;
  return true;
}

void PhoenixAudioManager::clearReverbBuffers() {
  if (!_reverbMem) return;
  static const uint32_t total = 1117U + 1277U + 1423U + 1559U + 337U + 421U + 353U + 443U;
  memset(_reverbMem, 0, (size_t)total * sizeof(int16_t));
  memset(_reverbCombPos, 0, sizeof(_reverbCombPos));
  memset(_reverbCombLp, 0, sizeof(_reverbCombLp));
  memset(_reverbApPosL, 0, sizeof(_reverbApPosL));
  memset(_reverbApPosR, 0, sizeof(_reverbApPosR));
...
void PhoenixAudioManager::refreshReverbCache() {
  if (!_reverbMem) { _reverbCacheReady = false; return; }

  // Snapshot volatile UI/control targets once per audio block. The resulting
  // cache is owned by AudioTask/Core1 and therefore cannot tear mid-sample.
  const uint8_t size = _reverbSize;
  const uint8_t decay = _reverbDecay;
  const uint8_t damp = _reverbDamp;
  if (_reverbCacheReady && size == _reverbCachedSize &&
      decay == _reverbCachedDecay && damp == _reverbCachedDamp) return;

  static const uint16_t combMax[4] = {1117,1277,1423,1559};
  static const uint16_t apMaxL[2] = {337,421};
  static const uint16_t apMaxR[2] = {353,443};

  uint32_t off = 0;
  for (uint8_t k=0; k<4; ++k) { _reverbCombMem[k] = _reverbMem + off; off += combMax[k]; }
  for (uint8_t k=0; k<2; ++k) { _reverbApMemL[k] = _reverbMem + off; off += apMaxL[k]; }
  for (uint8_t k=0; k<2; ++k) { _reverbApMemR[k] = _reverbMem + off; off += apMaxR[k]; }

  // Exact DEV2g integer formulas, moved out of the per-sample render path.
  const uint32_t sizePct = 55U + ((uint32_t)size * 45U) / 100U;
  for (uint8_t k=0; k<4; ++k) {
    uint32_t len = ((uint32_t)combMax[k] * sizePct) / 100U;
    if (len < 64U) len = 64U;
    _reverbCombLen[k] = len;
  }
  for (uint8_t k=0; k<2; ++k) {
    uint32_t lenL = ((uint32_t)apMaxL[k] * sizePct) / 100U;
    uint32_t lenR = ((uint32_t)apMaxR[k] * sizePct) / 100U;
    if (lenL < 32U) lenL = 32U;
    if (lenR < 32U) lenR = 32U;
    _reverbApLenL[k] = lenL;
    _reverbApLenR[k] = lenR;
  }

  int32_t fbQ15 = 18022 + ((int32_t)decay * 134);
  if (fbQ15 > 31457) fbQ15 = 31457;
  _reverbCachedFbQ15 = fbQ15;
  _reverbCachedLpShift = (uint8_t)(2U + ((uint32_t)damp * 4U / 101U));
  _reverbCachedSize = size;
  _reverbCachedDecay = decay;
  _reverbCachedDamp = damp;
  _reverbCacheReady = true;
}

void PhoenixAudioManager::processReverb(int16_t send, int16_t &wetL, int16_t &wetR) {
  wetL = wetR = 0;
  if (_reverbMix == 0 || !_reverbMem || !_reverbCacheReady) return;

  int32_t sum=0;
  const int32_t fbQ15 = _reverbCachedFbQ15;
  const uint8_t lpShift = _reverbCachedLpShift;
  for(uint8_t k=0;k<4;k++){
    const uint32_t len = _reverbCombLen[k];
    uint32_t pos=_reverbCombPos[k]; if(pos>=len)pos=0;
    int32_t y=_reverbCombMem[k][pos];
    _reverbCombLp[k] += (y - _reverbCombLp[k]) >> lpShift;
    const int32_t w=(int32_t)send + ((_reverbCombLp[k]*fbQ15)>>15);
    _reverbCombMem[k][pos]=phxClamp16(w); if(++pos>=len)pos=0; _reverbCombPos[k]=pos; sum+=y;
  }
  int32_t l=sum>>2, r=sum>>2;
  const int32_t g=16384; // 0.5 allpass coefficient
  for(uint8_t k=0;k<2;k++){
    const uint32_t lenL = _reverbApLenL[k];
    uint32_t p=_reverbApPosL[k]; if(p>=lenL)p=0; int32_t b=_reverbApMemL[k][p]; int32_t y=b-((l*g)>>15);
    _reverbApMemL[k][p]=phxClamp16(l+((y*g)>>15)); if(++p>=lenL)p=0; _reverbApPosL[k]=p; l=y;
    const uint32_t lenR = _reverbApLenR[k];
    p=_reverbApPosR[k]; if(p>=lenR)p=0; b=_reverbApMemR[k][p]; y=b-((r*g)>>15);
    _reverbApMemR[k][p]=phxClamp16(r+((y*g)>>15)); if(++p>=lenR)p=0; _reverbApPosR[k]=p; r=y;
  }
  wetL=phxClamp16((l*(int32_t)_reverbMix)/100);
  wetR=phxClamp16((r*(int32_t)_reverbMix)/100);
}
...
void PhoenixAudioManager::startEchoTail() {
  // v1.1.0 DEV2c: this flag now represents the shared Echo/Reverb FX tail.
  // The old code only kept processing while Echo was active, which cut a
  // reverb-only tail immediately and could stop a long echo before its first
  // repeat arrived.
  const bool echoPossible = (_echoBuffer && _echoCapacityFrames > 1 && _echoMix > 0);
  const bool reverbPossible = (_reverbMem && _reverbMix > 0);
  _echoTailActive = echoPossible || reverbPossible;
  _echoTailQuietFrames = 0;
}
...
void PhoenixAudioManager::setReverbParams(uint8_t size, uint8_t decay, uint8_t damp, uint8_t mix) {
  _reverbSize = (uint8_t)constrain((int)size,0,100);
  _reverbDecay = (uint8_t)constrain((int)decay,0,100);
  _reverbDamp = (uint8_t)constrain((int)damp,0,100);
  _reverbMix = (uint8_t)constrain((int)mix,0,100);
}

void PhoenixAudioManager::setReverbSend(uint8_t slot, uint8_t send) {
  _slotReverbSend[safeSlot(slot)] = (uint8_t)constrain((int)send,0,100);
}
...
void PhoenixAudioManager::resetDiagnostics() {
  ...
  _underruns = 0;
  _peakTracePeakUs = 0;
  _peakTracePrintedSeq = _peakTraceSeq;
  _voiceStealCount = 0;
...
void PhoenixAudioManager::setKeygroupMapping(uint8_t slot,uint8_t group,bool enabled,uint8_t low,uint8_t high,uint8_t root,uint8_t level,int8_t pan){
  slot=safeSlot(slot);group&=15;MultisampleKeygroup &kg=_keygroups[slot][group];const uint8_t oldRoot=kg.rootNote;kg.enabled=enabled;kg.lowNote=constrain((int)low,0,127);kg.highNote=constrain((int)high,0,127);if(kg.highNote<kg.lowNote)kg.highNote=kg.lowNote;kg.rootNote=constrain((int)root,0,127);setKeygroupLayerMapping(slot,group,0,enabled,1,127,level,pan);if(kg.rootNote!=oldRoot)retuneActiveKeygroupVoices(slot,group);
}
...
void PhoenixAudioManager::retuneActiveKeygroupVoices(uint8_t slot,uint8_t group,uint16_t smoothFrames){
  slot=safeSlot(slot);group&=15;const MultisampleKeygroup &kg=_keygroups[slot][group];
  const float bendSemis=((float)_slotPitchBend[slot]/8192.0f)*(float)_slotPitchBendRange[slot];
  const float bendRatio=powf(2.0f,bendSemis/12.0f);
  for(uint8_t i=0;i<VOICE_COUNT;++i){
    InstrumentVoice &v=_voices[i];if(!v.active||v.slot!=slot||v.keygroup!=group)continue;
    int cents=(int)_slotCoarse[slot]*100+(int)_slotFine[slot]+(int)kg.transposeSemitone*100+(int)kg.fineCent;
    if(_slotPitchTrack[slot]){int semis=(int)v.note-(int)kg.rootNote+((int)_slotOctave[slot]*12);semis=constrain(semis,-48,48);cents+=(semis*100*(int)kg.keytrackPct)/100;}
    float ratio=powf(2.0f,(float)cents/1200.0f);ratio=constrain(ratio,0.25f,4.0f);
    uint32_t newBase=(uint32_t)(ratio*65536.0f+0.5f);if(!newBase)newBase=1;uint32_t newTarget=(uint32_t)((float)newBase*bendRatio+0.5f);if(!newTarget)newTarget=1;
    v.baseIncQ16=newBase;v.targetIncQ16=newTarget;
    if(smoothFrames>0){v.glideFramesLeft=smoothFrames;v.glideStepQ16=((int32_t)v.targetIncQ16-(int32_t)v.incQ16)/(int32_t)smoothFrames;if(v.glideStepQ16==0&&v.incQ16!=v.targetIncQ16)v.glideStepQ16=(v.targetIncQ16>v.incQ16)?1:-1;}
    else{v.incQ16=v.targetIncQ16;v.glideFramesLeft=0;v.glideStepQ16=0;}
  }
}

void PhoenixAudioManager::setKeygroupTranspose(uint8_t slot,uint8_t group,int8_t semitone){slot=safeSlot(slot);group&=15;auto &kg=_keygroups[slot][group];const int8_t nv=(int8_t)constrain((int)semitone,-24,24);if(kg.transposeSemitone==nv)return;kg.transposeSemitone=nv;retuneActiveKeygroupVoices(slot,group);}
void PhoenixAudioManager::setKeygroupFineCent(uint8_t slot,uint8_t group,int16_t cents){slot=safeSlot(slot);group&=15;auto &kg=_keygroups[slot][group];const int16_t nv=(int16_t)constrain((int)cents,-100,100);if(kg.fineCent==nv)return;kg.fineCent=nv;retuneActiveKeygroupVoices(slot,group);}
void PhoenixAudioManager::setKeygroupKeytrackPct(uint8_t slot,uint8_t group,uint8_t pct){slot=safeSlot(slot);group&=15;auto &kg=_keygroups[slot][group];const uint8_t nv=(uint8_t)constrain((int)pct,0,100);if(kg.keytrackPct==nv)return;kg.keytrackPct=nv;retuneActiveKeygroupVoices(slot,group);}
...
void PhoenixAudioManager::clearBankContent() {
  stopTransport();
  allNotesOff();
  releaseAllSequencerGates();
  _midiCmdRead = _midiCmdWrite;
  _seqCmdRead = _seqCmdWrite;
  _previewFrames = 0;
  _previewPos = 0;
  _playRangeActive = false;
  _recordPos = 0;
...
// v1.1.0 MULTISAMPLE BUFFERED LOAD PERF1
// Streaming read cache for multisample WAV imports. The legacy path decoded each
// PCM channel with a separate 1/2/3/4-byte File::read(), causing millions of
// tiny FAT/SPI transactions for large stereo WAVs. This reader keeps the exact
// PCM decode/resample semantics but refills from SD in 8 KiB blocks.
struct PhxBufferedPcmReader {
  File &file;
  uint8_t *buffer;
  size_t capacity;
  size_t position;
  ...
  explicit PhxBufferedPcmReader(File &f)
    : file(f), buffer(nullptr), capacity(0), position(0), filled(0) {}

  bool begin() {
    // Prefer internal 8-bit RAM for the small streaming cache. If 8 KiB is not
    // available, retry with 4 KiB. If both allocations fail, readExact() falls
    // back to the former direct File::read() behaviour instead of failing a bank.
    capacity = 8192U;
    buffer = (uint8_t*)heap_caps_malloc(capacity, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (!buffer) {
      capacity = 4096U;
      buffer = (uint8_t*)heap_caps_malloc(capacity, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    }
  ...
  bool readExact(uint8_t *dst, size_t bytes) {
    if (bytes == 0) return true;
    if (!buffer || capacity == 0) {
      return file.read(dst, bytes) == (int)bytes;
    }
    while (bytes) {
      if (position >= filled) {
        filled = file.read(buffer, capacity);
        position = 0;
        if (filled == 0) return false;
      }
      const size_t available = filled - position;
      const size_t take = (bytes < available) ? bytes : available;
      memcpy(dst, buffer + position, take);
      dst += take;
      position += take;
      bytes -= take;
    }
...
static bool phxReadPcmFrameBuffered(PhxBufferedPcmReader &r, uint16_t channels, uint16_t bits, int16_t &out) {
  const size_t bytesPerSample = (size_t)bits >> 3;
  const size_t frameBytes = bytesPerSample * (size_t)channels;
  uint8_t frame[8]; // max: stereo 32-bit PCM
  if (frameBytes == 0 || frameBytes > sizeof(frame) || !r.readExact(frame, frameBytes)) return false;

  int32_t sum = 0;
  for (uint16_t c = 0; c < channels; ++c) {
    const uint8_t *b = frame + (size_t)c * bytesPerSample;
    int32_t v = 0;
    if (bits == 8) {
      v = ((int32_t)b[0] - 128) << 8;
    } else if (bits == 16) {
...
bool PhoenixAudioManager::importWavToSlot(const char *path, uint8_t slot, char *status, size_t statusLen) {
  ...
  for (uint32_t out = 0; out < outFrames; ++out) {
    ...
    if ((out & 0x3FFUL) == 0) {
      reportProgress((uint8_t)(((uint64_t)(out + 1U) * 100ULL) / outFrames));
      // v1.0.4 WDT FIX: WAV import runs in PhoenixControl on Core 0. delay(0)
      // can starve IDLE0 during long SD/resampling loops. Yield one real RTOS
      // tick so IDLE0 can service the ESP32 task watchdog reliably.
...
bool PhoenixAudioManager::importWavToKeygroupVariant(const char *path,uint8_t slot,uint8_t group,uint8_t layer,uint8_t variant,char *status,size_t statusLen){
  slot=safeSlot(slot);
  group&=15;
  layer%=MAX_VELOCITY_LAYERS;
  variant%=MAX_RR_VARIANTS;
  PHX_INFO_PRINTF("V1.1.0 MSBUF1 LOAD begin S%u G%u L%u R%u path=%s\n",(unsigned)(slot+1),(unsigned)(group+1),(unsigned)(layer+1),(unsigned)(variant+1),path?path:"?");
  vTaskDelay(pdMS_TO_TICKS(2));

  File f=SD.open(path,FILE_READ);
  if(!f){if(status)snprintf(status,statusLen,"OPEN ERROR");return false;}

  uint16_t ch=0,bits=0;
  uint32_t sr=0,dataOff=0,dataLen=0;
  if(!phxReadWavHeaderGeneric(f,ch,sr,bits,dataOff,dataLen)){f.close();if(status)snprintf(status,statusLen,"BAD WAV");return false;}

  const uint32_t bpf=(bits/8U)*ch;
  const uint32_t inFrames=bpf?dataLen/bpf:0;
  const uint32_t outFrames=(uint32_t)(((uint64_t)inFrames*32000ULL)/sr);
  if(!outFrames){f.close();if(status)snprintf(status,statusLen,"EMPTY WAV");return false;}

  int16_t*buf=(int16_t*)ps_malloc(outFrames*sizeof(int16_t));
  if(!buf){f.close();if(status)snprintf(status,statusLen,"NO PSRAM");return false;}
  if(!f.seek(dataOff)){free(buf);f.close();if(status)snprintf(status,statusLen,"SEEK ERROR");return false;}

  PhxBufferedPcmReader reader(f);
  reader.begin();
  uint32_t src=0;
  int16_t cur=0;
  reportProgress(0);
  for(uint32_t o=0;o<outFrames;++o){
    const uint32_t target=(uint32_t)(((uint64_t)o*sr)/32000ULL);
    while(src<=target){
      if(!phxReadPcmFrameBuffered(reader,ch,bits,cur))break;
      ++src;
    }
    buf[o]=cur;
    if((o&0x3ffU)==0U){
      reportProgress((uint8_t)(((uint64_t)(o+1U)*100ULL)/outFrames));
      // Preserve the validated Core-0 watchdog yield cadence from the release.
      vTaskDelay(pdMS_TO_TICKS(2));
    }
  }
  reportProgress(100);
  ...
  MultisampleKeygroup &kg=_keygroups[slot][group];
  VelocityLayer &vl=kg.layer[layer];
  RoundRobinVariant &rv=vl.rr[variant];
  if(rv.buffer)free(rv.buffer);
  rv.buffer=buf;
  rv.frames=outFrames;
  strncpy(rv.path,path,sizeof(rv.path));
  rv.path[sizeof(rv.path)-1]=0;
  vl.enabled=true;
  if(layer==0&&!kg.enabled){kg.lowNote=0;kg.highNote=127;kg.rootNote=60;}
  kg.enabled=true;
  if(vl.velocityHigh<vl.velocityLow){
    vl.velocityLow=(layer==0)?1:(uint8_t)(layer*43+1);
    vl.velocityHigh=(layer==2)?127:(uint8_t)((layer+1)*43);
  }
  if(variant>0&&vl.rrMode==0)vl.rrMode=(uint8_t)(variant+1);
  updateVelocityLayerPan(vl);
  if(status)snprintf(status,statusLen,"S%u G%02u L%u R%u",slot+1,group+1,layer+1,variant+1);
  PHX_INFO_PRINTF("V1.1.0 MSBUF1 LOAD ok S%u G%u L%u R%u frames=%lu\n",(unsigned)(slot+1),(unsigned)(group+1),(unsigned)(layer+1),(unsigned)(variant+1),(unsigned long)outFrames);
  vTaskDelay(pdMS_TO_TICKS(2));
  return true;
}
...
bool PhoenixAudioManager::writeMultisampleConfigFile(const char *path, bool *anyOut) const {
  ...
  delay(1); // v1.1.1 FIX1: ensure IDLE0 runs during long multisample metadata saves
  ...
  for (uint8_t s = 0; s < SLOT_COUNT; ++s) {
    for (uint8_t g = 0; g < MAX_KEYGROUPS; ++g) {
      ...
      delay(1); // v1.1.1 FIX1: one real tick per keygroup header
      ...
      for (uint8_t l = 0; l < MAX_VELOCITY_LAYERS; ++l) {
        ...
        delay(1); // v1.1.1 FIX1: cooperative yield after each written velocity layer
      }
      delay(1);
    }
  }
  ...
  f.flush();
  delay(1); // v1.1.1 FIX1: FAT flush may follow hundreds of tiny writes
...
bool PhoenixAudioManager::loadMultisampleConfig(const char *dir){
  ...
  char p[160];
  snprintf(p,sizeof(p),"%s/MULTISAMPLE.CFG",dir);

  // v1.1.0 FINAL LOAD-PROGRESS-FIX1:
  // Count the actual sample imports first.  Previously every imported WAV reused
  // the complete parent progress range (for bank loading normally 60..95%).
  // The first WAV therefore reached 95% and all later lower percentages were
  // rejected by the monotonic UI filter.  The loading screen appeared frozen.
  uint16_t importCount=0;
  {
    File countFile=SD.open(p,FILE_READ);
    if(!countFile){PHX_INFO_PRINTLN("V1.0.3 MS CFG absent");return false;}
    int countSlot=-1,countGroup=-1,countLayer=-1;
    char countLine[196];
    uint16_t countLines=0;
    while(countFile.available()){
      size_t n=countFile.readBytesUntil('\n',countLine,sizeof(countLine)-1);
      countLine[n]=0;
      phxTrimLine(countLine);
      if((++countLines & 0x0FU)==0U) vTaskDelay(pdMS_TO_TICKS(1));
      if(!countLine[0]||countLine[0]=='#')continue;
      if(countLine[0]=='['){
        unsigned a=0,b=0,c=0;
        if(sscanf(countLine,"[SLOT%u_GROUP%u_LAYER%u]",&a,&b,&c)==3&&a>=1&&a<=4&&b>=1&&b<=16&&c>=1&&c<=3){countSlot=a-1;countGroup=b-1;countLayer=c-1;}
        else if(sscanf(countLine,"[SLOT%u_GROUP%u]",&a,&b)==2&&a>=1&&a<=4&&b>=1&&b<=16){countSlot=a-1;countGroup=b-1;countLayer=-1;}
        continue;
      }
      char*eq=strchr(countLine,'=');
      if(!eq||countSlot<0||countGroup<0)continue;
      *eq=0;
      char*key=countLine;
      phxTrimLine(key);
      if(countLayer<0){
        if(!strcmp(key,"FILE"))++importCount;
      }else{
        if(!strcmp(key,"FILE"))++importCount;
        else if(!strncmp(key,"RR",2)&&strstr(key,"_FILE")){
          const int r=atoi(key+2)-1;
          if(r>=0&&r<4)++importCount;
        }
      }
    }
    countFile.close();
  ...
  if(!f){PHX_INFO_PRINTLN("V1.0.3 MS CFG absent");return false;}

  const uint8_t parentBase=_progressBase;
  const uint8_t parentSpan=_progressSpan;
  uint16_t importIndex=0;
  auto importMapped = [&](const char* wavPath,uint8_t slot,uint8_t group,uint8_t layer,uint8_t variant)->bool {
    if(importCount>0){
      const uint16_t startPct=(uint16_t)parentBase+(((uint32_t)parentSpan*importIndex)/importCount);
      const uint16_t endPct=(uint16_t)parentBase+(((uint32_t)parentSpan*(importIndex+1U))/importCount);
      _progressBase=(uint8_t)min((uint16_t)100U,startPct);
      _progressSpan=(uint8_t)((endPct>startPct)?(endPct-startPct):0U);
      reportProgress(0);
    }
    char st[32];
    const bool ok=importWavToKeygroupVariant(wavPath,slot,group,layer,variant,st,sizeof(st));
    if(importIndex<importCount)++importIndex;
    return ok;
  };
  ...
  while(f.available()){
    ...
    if(cl<0){
      if(!strcmp(key,"FILE")){if(!importMapped(val,(uint8_t)cs,(uint8_t)cg,0,0))importError=true;kg.layer[0].velocityLow=1;kg.layer[0].velocityHigh=127;}
      ...
      if(!strcmp(key,"FILE")){if(!importMapped(val,(uint8_t)cs,(uint8_t)cg,(uint8_t)cl,0))importError=true;}
      else if(!strncmp(key,"RR",2)&&strstr(key,"_FILE")){int r=atoi(key+2)-1;if(r>=0&&r<4){if(!importMapped(val,(uint8_t)cs,(uint8_t)cg,(uint8_t)cl,(uint8_t)r))importError=true;}}
  ...
  _progressBase=parentBase;
  _progressSpan=parentSpan;
  reportProgress(100);
  PHX_INFO_PRINTF("V1.0.3 MS CFG %s lines=%u imports=%u\n",importError?"IMPORT_ERROR":"OK",(unsigned)lineCount,(unsigned)importCount);
  return !importError;
}
...
static void phxWriteProjectMetadata(File &cfg, const PhoenixAudioManager *a) {
  ...
  cfg.printf("REVERB_SIZE=%u\n", (unsigned)a->reverbSize());
  cfg.printf("REVERB_DECAY=%u\n", (unsigned)a->reverbDecay());
  cfg.printf("REVERB_DAMP=%u\n", (unsigned)a->reverbDamp());
  cfg.printf("REVERB_MIX=%u\n", (unsigned)a->reverbMix());
  ...
  // v1.1.1 FIX1: BANK.CFG contains many small FAT/SPI writes. Block PhoenixControl
  // for one RTOS tick at safe boundaries so IDLE0 can service the task watchdog.
  delay(1);
  for (uint8_t i = 0; i < PhoenixAudioManager::SLOT_COUNT; ++i) {
    ...
    cfg.printf("REVERB_SEND=%u\n", (unsigned)a->reverbSend(i));
    ...
    cfg.printf("REPLAY_REVERSE=%u\n", a->lastReplayReverse() ? 1 : 0);
    delay(1); // v1.1.1 FIX1: one cooperative tick per complete slot section
  }
}
...
bool PhoenixAudioManager::loadProjectMetadataFromSD(const char *dir) {
  ...
  // v1.1.0 DEV2a: BANK15 compatibility. Reverb sends are zero and globals use safe defaults.
  _reverbSize=55; _reverbDecay=55; _reverbDamp=45; _reverbMix=20;
  for (uint8_t i = 0; i < SLOT_COUNT; ++i) { _slotEchoSend[i] = 0; _slotReverbSend[i] = 0; _slotSampleGainPct[i]=100; _slotVelocityAmount[i]=100; _slots[i].trimEnabled=false; _slots[i].trimUndoValid=false; _slots[i].trimUndoSampleStart=0; _slots[i].trimUndoLoopStart=0; _slots[i].trimUndoLoopEnd=0; _slots[i].trimUndoSampleEnd=0; _slots[i].dcCorrectionEnabled=false; _slots[i].normalizeEnabled=false; _slots[i].dcOffset=0; _slots[i].normalizeGainQ16=65536UL; setMixerParams(i,100,0); setFilterParams(i,100,0,0,0,0); setFilterEnvelopeParams(i,0,250,0,300); }
  ...
  while (cfg.available()) {
    ...
    if (currentSlot < 0) {
      ...
      else if (strcmp(key, "ECHO_DELAY_MS") == 0) _echoDelayMs = (uint16_t)constrain((int)phxToLong(val, _echoDelayMs), 50, 2000);
      ...
      else if (strcmp(key, "REVERB_SIZE") == 0) _reverbSize = (uint8_t)constrain((int)phxToLong(val, 55), 0, 100);
      else if (strcmp(key, "REVERB_DECAY") == 0) _reverbDecay = (uint8_t)constrain((int)phxToLong(val, 55), 0, 100);
      else if (strcmp(key, "REVERB_DAMP") == 0) _reverbDamp = (uint8_t)constrain((int)phxToLong(val, 45), 0, 100);
      else if (strcmp(key, "REVERB_MIX") == 0) _reverbMix = (uint8_t)constrain((int)phxToLong(val, 20), 0, 100);
    ...
    else if (strcmp(key, "REVERB_SEND") == 0) _slotReverbSend[currentSlot] = (uint8_t)constrain((int)phxToLong(val, _slotReverbSend[currentSlot]), 0, 100);
...
static inline void phxC030Yield() {
  // delay(0) does not guarantee CPU0's priority-0 IDLE task gets scheduled.
  // A real one-tick sleep is intentional here: bank storage is not realtime audio.
  delay(1);
}

// v1.1.1 BANK-SAVE-PERF1:
// Scan a bank directory once and reuse the result instead of issuing many
// independent SD.exists()/SD.remove() metadata operations. On the validated
// hardware those metadata calls dominate save time (~0.2 s each).
struct PhxC030DirState {
  uint8_t targetMask;
  uint8_t backupMask;
  uint8_t tempMask;
  uint32_t targetSize[PHX_C030_TARGET_COUNT];
  bool journal;
  bool journalNew;
};

static bool phxC030InspectDir(const char *dir, PhxC030DirState &state) {
  ...
  for (;;) {
    ...
    if (!entry.isDirectory()) {
      ...
      base = base ? (base + 1) : (full ? full : "");

      if (!strcmp(base, "BANK.C30TXN")) state.journal = true;
      else if (!strcmp(base, "BANK.C30NEW")) state.journalNew = true;

      char expected[64];
      for (uint8_t i = 0; i < PHX_C030_TARGET_COUNT; ++i) {
        if (!strcmp(base, PHX_C030_TARGET_NAMES[i])) {
          state.targetMask |= (uint8_t)(1U << i);
          state.targetSize[i] = (uint32_t)entry.size();
          break;
        }
        snprintf(expected, sizeof(expected), "%s.C30BAK", PHX_C030_TARGET_NAMES[i]);
        if (!strcmp(base, expected)) {
          state.backupMask |= (uint8_t)(1U << i);
          break;
        }
        snprintf(expected, sizeof(expected), "%s.C30TMP", PHX_C030_TARGET_NAMES[i]);
        if (!strcmp(base, expected)) {
          state.tempMask |= (uint8_t)(1U << i);
          break;
        }
      }
    }
    entry.close();
    if ((++scanned & 0x07U) == 0U) phxC030Yield();
    else delay(0);
  }
  root.close();
  phxC030Yield();
...
static inline uint64_t phxC030NowUs() {
  return (uint64_t)esp_timer_get_time();
}

static void phxC030Trace(const char *phase, uint64_t startUs, uint64_t &lastUs) {
  const uint64_t nowUs = phxC030NowUs();
  PHX_INFO_PRINTF("PHX111P2 SAVE %s dt=%llu us total=%llu us\n",
                  phase ? phase : "?",
                  (unsigned long long)(nowUs - lastUs),
                  (unsigned long long)(nowUs - startUs));
  lastUs = nowUs;
  phxC030Yield();
}
...
static void phxC030CleanupArtifacts(const char *dir) {
  char p[176];
  for (uint8_t i = 0; i < PHX_C030_TARGET_COUNT; ++i) {
    phxC030Path(p, sizeof(p), dir, i, ".C30TMP"); SD.remove(p); phxC030Yield();
    phxC030Path(p, sizeof(p), dir, i, ".C30BAK"); SD.remove(p); phxC030Yield();
  }
  snprintf(p, sizeof(p), "%s/BANK.C30NEW", dir); SD.remove(p); phxC030Yield();
}
...
static bool phxC030RecoverBankTransaction(const char *dir) {
  ...
  const bool journalExists = SD.exists(journal);
  phxC030Yield();
  if (!journalExists) {
    // PERF1 fast path: without a journal there is no live transaction to roll back.
    // Stale .C30BAK/.C30TMP files can only be post-commit leftovers. They are
    // harmless and are removed lazily only if they can conflict with the next save.
    // Avoiding 13 unconditional FAT remove operations saves several seconds.
    return true;
  }
  ...
  for (uint8_t i = 0; i < PHX_C030_TARGET_COUNT; ++i) {
    ...
    if (existedBefore) {
      ...
      const bool backupExists = SD.exists(backup); phxC030Yield();
      if (backupExists) {
        const bool targetExists = SD.exists(target); phxC030Yield();
        if (targetExists) { SD.remove(target); phxC030Yield(); }
      ...
      const bool targetExists = SD.exists(target); phxC030Yield();
      if (targetExists) { if (!SD.remove(target)) ok = false; phxC030Yield(); }
      const bool backupExists = SD.exists(backup); phxC030Yield();
      if (backupExists) { SD.remove(backup); phxC030Yield(); }
    }
    SD.remove(temp);
    phxC030Yield();
  }
  if (ok) { SD.remove(journal); phxC030Yield(); }
...
static bool phxC030WriteJournal(const char *dir, uint8_t oldMask) {
  ...
  // PERF1: caller preflight guarantees BANK.C30TXN is absent and removes
  // BANK.C30NEW only when it really exists. Do not spend two FAT metadata
  // operations deleting files that are normally absent.
  ...
  f.close();
  phxC030Yield();
  if (!ok) { SD.remove(temp); phxC030Yield(); return false; }
  if (!SD.rename(temp, journal)) { SD.remove(temp); phxC030Yield(); return false; }
  phxC030Yield();
  const bool exists = SD.exists(journal);
  phxC030Yield();
  return exists;
}
...
bool PhoenixAudioManager::saveBankToSD(const char *dir, bool allowIntentionalEmptyBank) {
  if (!dir || !dir[0]) return false;
  const uint64_t phxSaveStartUs = phxC030NowUs();
  uint64_t phxSaveLastUs = phxSaveStartUs;
  PHX_INFO_PRINTF("PHX111P2 SAVE T0 START dir=%s\n", dir);
  phxC030Yield();

  // PERF1 preflight: one directory walk replaces mkdir(existing), journal exists,
  // six target exists checks and the unconditional 13-file cleanup fast path.
  PhxC030DirState preflight;
  if (!phxC030InspectDir(dir, preflight)) {
    if (!SD.mkdir(dir)) return false;
    phxC030Yield();
    if (!phxC030InspectDir(dir, preflight)) return false;
  }

  if (preflight.journal) {
    if (!phxC030RecoverBankTransaction(dir)) return false;
    if (!phxC030InspectDir(dir, preflight)) return false;
  }

  // Backups without a live journal are post-commit leftovers and can be deleted
  // safely before a new transaction. Remove only files that the scan found.
  char prePath[176];
  for (uint8_t i = 0; i < PHX_C030_TARGET_COUNT; ++i) {
    if (!(preflight.backupMask & (1U << i))) continue;
    phxC030Path(prePath, sizeof(prePath), dir, i, ".C30BAK");
    SD.remove(prePath);
    phxC030Yield();
  }
  if (preflight.journalNew) {
    snprintf(prePath, sizeof(prePath), "%s/BANK.C30NEW", dir);
    SD.remove(prePath);
    phxC030Yield();
  }

  const uint8_t oldMask = preflight.targetMask;
  phxC030Trace("T1 PREFLIGHT", phxSaveStartUs, phxSaveLastUs);
  ...
  phxC030Trace("T2 WAV_STAGE", phxSaveStartUs, phxSaveLastUs);
  ...
  if (!hasBankData && !allowIntentionalEmptyBank) {
    // C030 safety remains the default: an unexplained empty RAM state must never
    // erase an existing bank accidentally. Only PhoenixOS may explicitly opt in
    // after the user has executed DISK UTILITIES -> NEW BANK.
    phxC030CleanupArtifacts(dir);
    return false;
  }
  // Intentional empty NEW BANK: continue transaction with BANK.CFG as the only
  // audio-bank member. Existing SLOT*.WAV/MULTISAMPLE.CFG files are therefore
  // removed transactionally if the user deliberately overwrites a used bank.
  ...
  phxC030Trace("T3 MS_STAGE", phxSaveStartUs, phxSaveLastUs);
  ...
  phxC030Trace("T4 BANKCFG_STAGE", phxSaveStartUs, phxSaveLastUs);

  // PERF2 pre-commit invariant: every desired member must have been completely
  // staged, closed and assigned a non-zero size before the old generation is
  // made non-authoritative. WAV and BANK.CFG staging already perform explicit
  // size verification; MULTISAMPLE.CFG is reopened after close and its size read.
  uint8_t desiredMask = 0U;
  for (uint8_t i = 0; i < PHX_C030_TARGET_COUNT; ++i) {
    if (!desired[i]) continue;
    desiredMask |= (uint8_t)(1U << i);
    if (stagedSize[i] == 0U) {
      phxC030CleanupArtifacts(dir);
      return false;
    }
  ...
  // oldMask was captured by the single preflight directory scan.
  if (!phxC030WriteJournal(dir, oldMask)) { phxC030CleanupArtifacts(dir); return false; }
  phxC030Trace("T5 JOURNAL", phxSaveStartUs, phxSaveLastUs);
  ...
  for (uint8_t i = 0; i < PHX_C030_TARGET_COUNT; ++i) {
    ...
    phxC030Path(target, sizeof(target), dir, i, "");
    phxC030Path(backup, sizeof(backup), dir, i, ".C30BAK");
    // PERF1 fast path: preflight already removed stale backups that actually
    // existed. Try rename directly. Only use remove+retry on an unexpected clash.
  ...
  phxC030Trace("T6 BACKUP_OLD", phxSaveStartUs, phxSaveLastUs);

  // Phase 3: install the complete new generation. Files not desired remain absent.
  // PERF2 removes PERF1's full post-install directory rescan. All payloads are
  // verified before the journal is written; a successful same-directory FAT rename
  // changes the directory name, not the file's cluster chain or size. The journal
  // remains present until *all* desired renames report success, so any interruption
  // before the commit point is still rolled back to the complete old generation.
  uint8_t installedMask = 0U;
  for (uint8_t i = 0; i < PHX_C030_TARGET_COUNT; ++i) {
    if (!desired[i]) continue;
    phxC030Path(target, sizeof(target), dir, i, "");
    phxC030Path(temp, sizeof(temp), dir, i, ".C30TMP");
    ...
    installedMask |= (uint8_t)(1U << i);
    phxC030Yield();
  }
  if (installedMask != desiredMask) {
    phxC030RecoverBankTransaction(dir);
    return false;
  }
  phxC030Trace("T7 INSTALL_NEW", phxSaveStartUs, phxSaveLastUs);
  ...
  phxC030Yield();
  phxC030Trace("T8 COMMIT", phxSaveStartUs, phxSaveLastUs);

  // Backups are now obsolete. PERF1 removes only backups that were actually
  // created from the old generation. Stale temps are harmless without a journal
  // and are overwritten/removed lazily if a later transaction needs them.
  for (uint8_t i = 0; i < PHX_C030_TARGET_COUNT; ++i) {
    if (!(oldMask & (1U << i))) continue;
    phxC030Path(backup, sizeof(backup), dir, i, ".C30BAK");
    SD.remove(backup);
  ...
  phxC030Trace("T9 CLEANUP_DONE", phxSaveStartUs, phxSaveLastUs);
  PHX_INFO_PRINTF("C030 PERF2 STORAGE COMMIT dir=%s oldmask=0x%02X newmask=0x%02X\n", dir,
                (unsigned)oldMask,
...
bool PhoenixAudioManager::loadBankFromSD(const char *dir) {
  ...
  // v1.1.0 FINAL LOAD-PROGRESS-FIX1: reserve the last percentages for
  // metadata, patterns, song and GUI synchronization instead of reaching 95%
  // on the first multisample WAV. loadMultisampleConfig() subdivides 60..90
  // over all configured WAV imports.
  _progressBase = 60; _progressSpan = 30; reportProgress(0);
  bool loadedMulti = loadMultisampleConfig(dir);
  _progressBase = 90; _progressSpan = 4; reportProgress(0);
  if (loadedAny || loadedMulti) loadProjectMetadataFromSD(dir);
  reportProgress(100); // audio stage complete at 94%; OS finishes 95..100
...
void PhoenixAudioManager::audioTask() {
  ...
  for (;;) {
    ...
    // v1.1.0 FINAL retained: update derived reverb constants at block rate, never per sample.
    // Keep this inside the measured render window so baseline load numbers
...
#if PHX_REVERB_TIMING_ENABLED
    // v1.1.0 FINAL: one diagnostic sample per audio block, never per frame.
    bool reverbTimingPending = true;
#endif
    ...
    uint8_t voiceReverbSendCache[VOICE_COUNT];
    ...
    if (_activeVoiceCount > 0) {
      for (uint8_t vi = 0; vi < VOICE_COUNT; ++vi) {
        ...
        voiceReverbSendCache[vi] = _slotReverbSend[mslot];
    ...
    for (int i = 0; i < wordsRead; i += 2) {
      ...
      if (tone != TONE_THRU) {
        ...
        if (doRecord) {
          ...
          int16_t revL16=0,revR16=0; processReverb(0,revL16,revR16);
          tx[i] = ((int32_t)phxSoftLimit16((int32_t)(inL >> 16)+revL16)) << 16;
          if (i + 1 < wordsRead) tx[i + 1] = ((int32_t)phxSoftLimit16((int32_t)(inR >> 16)+revR16)) << 16;
        }
        ...
        for (uint8_t vi = 0; vi < VOICE_COUNT; ++vi) {
          ...
          reverbSendMix += (leveled * (int32_t)voiceReverbSendCache[vi]) / 100;
          ++live;
        ...
        int16_t revL16=0, revR16=0;
#if PHX_REVERB_TIMING_ENABLED
        if (reverbTimingPending) {
          const uint32_t revC0 = esp_cpu_get_ccount();
          processReverb(phxSoftLimit16(reverbSendMix >> 1), revL16, revR16);
          _reverbProcessUs = (esp_cpu_get_ccount() - revC0) / 240UL;
          reverbTimingPending = false;
        } else {
          processReverb(phxSoftLimit16(reverbSendMix >> 1), revL16, revR16);
        }
#else
        processReverb(phxSoftLimit16(reverbSendMix >> 1), revL16, revR16);
#endif
        tx[i] = ((int32_t)phxSoftLimit16((int32_t)dryL16 + wet16 + revL16)) << 16;
        if (i + 1 < wordsRead) tx[i + 1] = ((int32_t)phxSoftLimit16((int32_t)dryR16 + wet16 + revR16)) << 16;
        ...
        if (live == 0 && (_echoMix > 0 || _reverbMix > 0)) startEchoTail();
        ...
        if (transportValid && pos < s.frames) {
          ...
          int16_t wet16 = processEcho(0, send16);
          int16_t revL16=0,revR16=0;
          processReverb(phxClamp16(((int32_t)leveled*(int32_t)_slotReverbSend[mslot])/100),revL16,revR16);
          tx[i] = ((int32_t)phxSoftLimit16((int32_t)dryL16+wet16+revL16)) << 16;
          if (i + 1 < wordsRead) tx[i + 1] = ((int32_t)phxSoftLimit16((int32_t)dryR16+wet16+revR16)) << 16;
        ...
        if (_echoTailActive) {
          // v1.1.0 DEV2c: keep BOTH effects alive after the source stops.
          // The previous 4000-frame (~125 ms) silence timeout killed any echo
          // whose delay was longer than 125 ms before its first repeat arrived.
          // It also made the reverb tail depend on Echo being active.
          int16_t tail16 = (_echoMix > 0) ? processEcho(0, 0) : 0;
          int16_t revL16=0,revR16=0;
          if (_reverbMix > 0) processReverb(0,revL16,revR16);
          tx[i] = ((int32_t)phxSoftLimit16((int32_t)tail16+revL16)) << 16;
          if (i + 1 < wordsRead) tx[i + 1] = ((int32_t)phxSoftLimit16((int32_t)tail16+revR16)) << 16;

          uint32_t a = (tail16 < 0) ? (uint32_t)(-(int32_t)tail16) : (uint32_t)tail16;
          uint32_t ar = (revL16 < 0) ? (uint32_t)(-(int32_t)revL16) : (uint32_t)revL16;
          if (ar > a) a = ar;
          ar = (revR16 < 0) ? (uint32_t)(-(int32_t)revR16) : (uint32_t)revR16;
          if (ar > a) a = ar;

          // Require continuous silence for at least one full Echo delay period
          // (plus 128 ms margin), otherwise long-delay gaps are mistaken for
          // the end of the tail. For reverb-only operation use 250 ms.
          uint32_t quietNeed = 8000U;
          if (_echoMix > 0) {
            uint32_t df = ((uint32_t)_echoDelayMs * 32000UL) / 1000UL;
            if (df + 4096U > quietNeed) quietNeed = df + 4096U;
          }
          if (a < 8U) {
            if (_echoTailQuietFrames < quietNeed) ++_echoTailQuietFrames;
            else stopEchoTail();
          } else {
            _echoTailQuietFrames = 0;
          }
    ...
    // v1.1.0 FINAL compact PEAKTRACE: capture context only for a NEW high-water mark
    // above 3.5 ms. Diagnostic-only per-sample XFade counters were removed
    // from the hotpath; no Serial output or allocation occurs on Core1.
    if (processUs >= 3500UL && processUs > _peakTracePeakUs) {
      uint8_t releasing = 0, multisample = 0, filterActive = 0;
      uint8_t vintageActive = 0, slotMask = 0;
      for (uint8_t vi = 0; vi < VOICE_COUNT; ++vi) {
        InstrumentVoice &tv = _voices[vi];
        if (!tv.active) continue;
        if (tv.releasing) ++releasing;
        if (tv.keygroup != 255U) ++multisample;
        if (!voiceFilterBypassCache[vi]) ++filterActive;
        if (!voiceVintageBypassCache[vi]) ++vintageActive;
        slotMask |= (uint8_t)(1U << safeSlot(tv.slot));
      }

      ++_peakTraceSeq; // odd = writer owns snapshot
      __sync_synchronize();
      _peakTracePeakUs = processUs;
      _peakTraceFrame = _frameCounter;
      _peakTraceVoices = histVoices;
      _peakTraceReleasing = releasing;
      _peakTraceMultisample = multisample;
      _peakTraceFilterActive = filterActive;
      _peakTraceVintageActive = vintageActive;
      _peakTraceSlotMask = slotMask;
      _peakTraceEchoMix = _echoMix;
      _peakTraceReverbMix = _reverbMix;
      _peakTraceReverbSize = _reverbSize;
      _peakTraceReverbDecay = _reverbDecay;
      _peakTraceReverbDamp = _reverbDamp;
      _peakTraceState = state;
      _peakTraceQuattroMode = _quattroMode;
      _peakTraceTailActive = _echoTailActive ? 1U : 0U;
      __sync_synchronize();
      ++_peakTraceSeq; // even = stable snapshot
    }
...
void PhoenixAudioManager::update(PhoenixEventBus &events) {
  ...
  if (now - _audioStatsPrintMs >= 1000UL) {
    ...
    // DEV3: print a stable compact snapshot only when a new >=3.5 ms
    // high-water block has been captured. Sequence odd/even guards prevent
    // Core0 from printing a partially updated snapshot.
    const uint32_t traceSeq1 = _peakTraceSeq;
    if (traceSeq1 != _peakTracePrintedSeq && (traceSeq1 & 1U) == 0U) {
      __sync_synchronize();
      const uint32_t tUs = _peakTracePeakUs;
      const uint32_t tFrame = _peakTraceFrame;
      const uint8_t tVoices = _peakTraceVoices;
      const uint8_t tRel = _peakTraceReleasing;
      const uint8_t tMs = _peakTraceMultisample;
      const uint8_t tFilter = _peakTraceFilterActive;
      const uint8_t tVintage = _peakTraceVintageActive;
      const uint8_t tSlots = _peakTraceSlotMask;
      const uint8_t tEcho = _peakTraceEchoMix;
      const uint8_t tRev = _peakTraceReverbMix;
      const uint8_t tRevSize = _peakTraceReverbSize;
      const uint8_t tRevDecay = _peakTraceReverbDecay;
      const uint8_t tRevDamp = _peakTraceReverbDamp;
      const uint8_t tState = _peakTraceState;
      const uint8_t tQuattro = _peakTraceQuattroMode;
      const uint8_t tTail = _peakTraceTailActive;
      __sync_synchronize();
      const uint32_t traceSeq2 = _peakTraceSeq;
      if (traceSeq1 == traceSeq2 && (traceSeq2 & 1U) == 0U) {
        Serial.printf("PHX PEAKTRACE us=%lu frame=%lu voices=%u rel=%u ms=%u filter=%u vintage=%u slots=0x%02X echo=%u reverb=%u size=%u decay=%u damp=%u tail=%u state=%u quattro=%u\n",
                      (unsigned long)tUs, (unsigned long)tFrame,
                      (unsigned)tVoices, (unsigned)tRel, (unsigned)tMs,
                      (unsigned)tFilter, (unsigned)tVintage,
                      (unsigned)tSlots, (unsigned)tEcho, (unsigned)tRev,
                      (unsigned)tRevSize, (unsigned)tRevDecay, (unsigned)tRevDamp,
                      (unsigned)tTail, (unsigned)tState, (unsigned)tQuattro);
        _peakTracePrintedSeq = traceSeq2;
      }
    }
