...
public:
  ...
  void stopTransport();
  // v1.1.0 DEV1: clear all bank-owned sample/multisample content in RAM.
  void clearBankContent();
  ...
  void setReverbParams(uint8_t size, uint8_t decay, uint8_t damp, uint8_t mix);
  void setReverbSend(uint8_t slot, uint8_t send);
  ...
  uint8_t reverbSize() const { return _reverbSize; }
  uint8_t reverbDecay() const { return _reverbDecay; }
  uint8_t reverbDamp() const { return _reverbDamp; }
  uint8_t reverbMix() const { return _reverbMix; }
  uint8_t reverbSend(uint8_t slot) const { return _slotReverbSend[safeSlot(slot)]; }
  uint32_t reverbProcessUs() const { return _reverbProcessUs; }
  ...
  // NEWBANK EMPTY SAVE FIX1: allow an intentionally initialized NEW BANK to be
  // committed with metadata only. Default remains false so an accidental empty
  // RAM state can never erase an existing bank.
  bool saveBankToSD(const char *dir, bool allowIntentionalEmptyBank = false);
...
private:
  ...
  void refreshReverbCache();
  void processReverb(int16_t send, int16_t &wetL, int16_t &wetR);
  ...
  void retuneActiveKeygroupVoices(uint8_t slot, uint8_t group, uint16_t smoothFrames = 128); // v1.0.10 live MS pitch edit
  int allocateVoice();
  ...
  uint8_t _slotReverbSend[SLOT_COUNT];
  ...
  // v1.1.0 FINAL high-water peak trace. AudioTask/Core1 captures a compact snapshot
  // only when a >=3500 us block establishes a new all-time peak. Core0 prints
  // the stable snapshot later; the audio task never calls Serial.
  volatile uint32_t _peakTraceSeq;
  volatile uint32_t _peakTracePeakUs;
  volatile uint32_t _peakTraceFrame;
  volatile uint8_t _peakTraceVoices;
  volatile uint8_t _peakTraceReleasing;
  volatile uint8_t _peakTraceMultisample;
  volatile uint8_t _peakTraceFilterActive;
  volatile uint8_t _peakTraceVintageActive;
  volatile uint8_t _peakTraceSlotMask;
  volatile uint8_t _peakTraceEchoMix;
  volatile uint8_t _peakTraceReverbMix;
  volatile uint8_t _peakTraceReverbSize;
  volatile uint8_t _peakTraceReverbDecay;
  volatile uint8_t _peakTraceReverbDamp;
  volatile uint8_t _peakTraceState;
  volatile uint8_t _peakTraceQuattroMode;
  volatile uint8_t _peakTraceTailActive;
  uint32_t _peakTracePrintedSeq;
  ...
  volatile uint8_t _reverbSize;
  volatile uint8_t _reverbDecay;
  volatile uint8_t _reverbDamp;
  volatile uint8_t _reverbMix;
  volatile uint32_t _reverbProcessUs;
  int16_t *_reverbMem;
  // v1.1.0 FINAL retained: AudioTask-owned Reverb FAST-CACHE. Targets remain volatile UI
  // parameters; derived delay lengths, feedback, damping and memory pointers
  // are refreshed once per 128-frame block only when a target changed.
  bool _reverbCacheReady;
  uint8_t _reverbCachedSize, _reverbCachedDecay, _reverbCachedDamp;
  int32_t _reverbCachedFbQ15;
  uint8_t _reverbCachedLpShift;
  int16_t *_reverbCombMem[4];
  int16_t *_reverbApMemL[2], *_reverbApMemR[2];
  uint32_t _reverbCombLen[4];
  uint32_t _reverbApLenL[2], _reverbApLenR[2];
  uint32_t _reverbCombPos[4];
  int32_t _reverbCombLp[4];
  uint32_t _reverbApPosL[2], _reverbApPosR[2];
