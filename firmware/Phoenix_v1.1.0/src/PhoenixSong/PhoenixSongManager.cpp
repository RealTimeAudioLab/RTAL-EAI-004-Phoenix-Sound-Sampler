...
void PhoenixSongManager::beginEmptySong() {
  // v1.1.0 DEV1a: true empty song state for NEW BANK.
  // Every entry is END, so Song Start cannot launch an implicit pattern.
  for (uint8_t i = 0; i < kMaxEntries; ++i) {
    _entries[i].pattern = 0;
    _entries[i].repeats = 1;
    _entries[i].end = true;
