...
: _screen(SCR_BOOT), _diagnosticsReturnScreen(SCR_MAIN), _sel(0), _bootStart(0), _about(false), _overwriteConfirm(false), _overwriteSlot(0), _recordReturnScreen(SCR_SOUND), _recordCompleteSlot(0), _recordCompleteSaveState(-1), _serviceMode(false), _toneRequest(-1), _audioTestParam(0), _audioTestEditMode(false), _audioTestWaveform(0), _audioTestFreqHz(440), _audioTestLevelDb(-12), _audioTestOutput(false), _audioTestChanged(false),
  ...
  _autoMapFolder[0] = 0;
  for (uint8_t i = 0; i < PHX_BROWSER_MAX; ++i) { _browserEntries[i].name[0] = 0; _browserEntries[i].isDir = false; _browserEntries[i].size = 0; _browserEntries[i].wavInfoValid = false; _browserEntries[i].wavInfoScanned = false; }
...
bool PhoenixScreenManager::allocateMultisamplePathCache() {
  ...
  PHX_INFO_PRINTF("MEM: multisample path cache %u bytes in %s\n",
...
void PhoenixScreenManager::setReverbStatus(uint8_t size, uint8_t decay, uint8_t damp, uint8_t mix, const uint8_t send[4]) {
  _reverbSize=(uint8_t)constrain((int)size,0,100); _reverbDecay=(uint8_t)constrain((int)decay,0,100);
  _reverbDamp=(uint8_t)constrain((int)damp,0,100); _reverbMix=(uint8_t)constrain((int)mix,0,100);
  for(uint8_t i=0;i<4;++i)_reverbSend[i]=send?(uint8_t)constrain((int)send[i],0,100):0;
}
...
void PhoenixScreenManager::browserGoRoot() {
  // v1.0.6: Re-entering WAV IMPORT at the root must be instant when the
  // current RAM cache already represents /PHOENIX/WAV. The directory itself
  // is not modified by importing a WAV into a Phoenix slot.
  if (_browserCacheValid && phxIsWavRoot(_browserPath)) {
    _browserSel = 0;
    ensureSelectedBrowserWavInfo();
    snprintf(_browserStatus, sizeof(_browserStatus), "%u ITEM(S) CACHED", (unsigned)_browserCount);
...
void PhoenixScreenManager::setMultisampleLoadStatus(const char *text) {
  if (!text) text = "";
  strncpy(_msLoadStatus, text, sizeof(_msLoadStatus));
  _msLoadStatus[sizeof(_msLoadStatus)-1] = 0;
}

void PhoenixScreenManager::invalidateSampleBrowserCache() {
  // v1.0.6: Call this only when external storage contents may have changed
  // (for example writable USB MSC). Normal slot import does not change WAV dir.
  _browserCacheValid = false;
}


bool PhoenixScreenManager::readWavInfo(const char *fullPath, BrowserEntry &entry) {
  entry.wavInfoScanned = true;
...
void PhoenixScreenManager::scanSampleBrowser() {
  ...
  for (uint8_t i = 0; i < PHX_BROWSER_MAX; ++i) {
    ...
    _browserEntries[i].wavInfoScanned = false;
  ...
  while (_browserCount < PHX_BROWSER_MAX) {
    ...
    if (phxBrowserShowName(bn, isDir)) {
      ...
      // v1.0.6: Do NOT open every WAV here. With large libraries this was the
      // main multi-second pause when entering/leaving WAV IMPORT. Metadata is
      // loaded lazily only for the currently selected WAV.
      _browserEntries[_browserCount].wavInfoValid = false;
      _browserEntries[_browserCount].wavInfoScanned = isDir;
      ++_browserCount;
    }
    entry.close();
  }
...
void PhoenixScreenManager::ensureSelectedBrowserWavInfo() {
  if (_browserCount == 0 || _browserSel >= _browserCount) return;
  BrowserEntry &e = _browserEntries[_browserSel];
  if (e.isDir || e.wavInfoScanned) return;
  char fullPath[sizeof(_browserPath) + 72];
  phxJoinPath(_browserPath, e.name, fullPath, sizeof(fullPath));
  e.wavInfoScanned = true;
  readWavInfo(fullPath, e);
}
...
bool PhoenixScreenManager::consumeReverbChanged() { bool r=_reverbChanged; _reverbChanged=false; return r; }
...
int PhoenixScreenManager::itemCountFor(ScreenID s) const {
  switch (s) {
    ...
    case SCR_EFFECTS: return 2;
    case SCR_ECHO: return 7;
    case SCR_REVERB: return 8;
    case SCR_LOOP: return 2;
    case SCR_SAMPLE_EDITOR: return 4;
    case SCR_TRIGGER: return 3;
    case SCR_DISK: return 6;
...
void PhoenixScreenManager::enterSelection() {
  ...
  if (_screen == SCR_MAIN) {
    ...
    if (_sel == 3) { go(SCR_EFFECTS); return; }
    if (_sel == 4) {
      go(SCR_DISK);
      _diskBankSelectAction = -1;
      _diskConfirmAction = -1;
      _diskBank = _activeBank;
      _diskAction = 3;
      return;
    }
  ...
  if (_screen == SCR_EFFECTS) { if(_sel==0){go(SCR_ECHO);_echoEditMode=false;} else {go(SCR_REVERB);_reverbEditMode=false;} return; }
  ...
  if (_screen == SCR_DISK) {
    // v1.0.12: first press on LOAD/SAVE/DELETE enters bank-number selection.
    // While selecting, encoder chooses BANK01..99 and a second press executes.
    if (_diskBankSelectAction < 0) {
      if (_sel <= 2) {
        _diskBankSelectAction = (_sel == 0) ? 1 : ((_sel == 1) ? 0 : 2);
        _diskBank = _activeBank;
        _diskConfirmAction = -1;
        _diskAction = 3;
        char msg[24];
      ...
      if (_sel == 3) {
        if (_diskConfirmAction != 4) { _diskConfirmAction = 4; setDiskStatus("NEW BANK? PUSH YES"); return; }
        _diskConfirmAction = -1; _diskAction = 4; return;
      }
      if (_sel == 4) { _diskConfirmAction = -1; browserGoRoot(); go(SCR_SAMPLE_BROWSER); return; }
      if (_sel == 5) { _diskConfirmAction = -1; go(SCR_USB_STORAGE); return; }
      return;
    }

    const int8_t action = _diskBankSelectAction;
    if (action == 1) {
    ...
    if (action == 0) {
      if (_diskBankUsed && _diskConfirmAction != 0) {
        ...
        setDiskStatus("OVERWRITE? PUSH YES");
        return;
      }
      _diskConfirmAction = -1;
      _diskAction = 0;
...
void PhoenixScreenManager::update(const PhoenixInputState &in) {
  ...
  if (_screen == SCR_LOOP) {
    ...
    // v1.1.0 DEV1b: standard Phoenix encoder workflow.
    // TURN = select parameter, PUSH = enter/leave edit mode, TURN while
    // editing = change value. The encoder push must never change a value.
    if (in.encPress) {
      _loopEditMode = !_loopEditMode;
      return;
    }
    ...
    // F1/F3 remain harmless legacy fallbacks for previous/next.
    if (in.f[1]) d = -1;
    if (in.f[3]) d = 1;
    if (d != 0) {
      if (!_loopEditMode) {
        int p = (int)_loopParam + encSign(d);
        if (p < 0) p = 1;
        if (p > 1) p = 0;
        _loopParam = (uint8_t)p;
        return;
      }
  ...
  if (_screen == SCR_DISK) {
    if (in.f[8]) {
      ...
      if (_diskBankSelectAction >= 0) {
        ...
        _diskBank = _activeBank;
        _diskAction = 3;
        setDiskStatus("SELECT OPERATION");
      } else {
        go(SCR_MAIN, 4);
      }
      return;
    ...
    if (_diskBankSelectAction >= 0) {
      ...
      if (in.f[6]) d = 1;
      if (d != 0) {
        // Encoder is the primary bank selector. Preserve magnitude so a fast
        // turn can move several banks while still wrapping BANK01..99.
        int bank = (int)_diskBank + (int)d;
        while (bank < 1) bank += 99;
        while (bank > 99) bank -= 99;
    ...
    // Menu mode: encoder moves LOAD/SAVE/DELETE/...; push opens selection.
    // Any navigation cancels a pending two-push confirmation (e.g. NEW BANK),
    // so a confirmation can never survive after the cursor moved elsewhere.
    if (in.encoderStep != 0 || in.f[1] || in.f[3]) {
      _diskConfirmAction = -1;
      setDiskStatus("SELECT OPERATION");
    }
  }
  ...
  if (_screen == SCR_SAMPLE_BROWSER) {
    ...
    if (in.f[8]) {
      _browserPreviewStopRequest = true;
      if (!browserGoUp()) { if(_keygroupImportMode||_autoMapBrowserMode){_keygroupImportMode=false;_autoMapBrowserMode=false;go(SCR_MULTISAMPLE);} else go(SCR_DISK,4); }
      return;
    }
  ...
  if (_screen == SCR_REVERB) {
    if (in.f[8]) { _reverbEditMode = false; go(SCR_EFFECTS, 1); return; }
    if (in.f[2]) {
      _reverbSize = 55;
      _reverbDecay = 55;
      _reverbDamp = 45;
      _reverbMix = 20;
      for (uint8_t i = 0; i < 4; ++i) _reverbSend[i] = 0;
      _reverbChanged = true;
      return;
    }
    if (in.encPress) {
      _reverbEditMode = !_reverbEditMode;
      return;
    }
    if (in.f[7]) {
      if (_playing || _recording) _stopRequest = true;
    ...
    if (delta != 0) {
      if (!_reverbEditMode) {
        int p = (int)_reverbParam + encSign(delta);
        if (p < 0) p = 7;
        if (p > 7) p = 0;
        _reverbParam = (uint8_t)p;
        return;
      }

      const int step = (in.encoderStep != 0) ? percentStepFromEncoder(delta) : delta;
      if (_reverbParam == 0) {
        int v = constrain((int)_reverbSize + step, 0, 100);
        if (v != (int)_reverbSize) { _reverbSize = (uint8_t)v; _reverbChanged = true; }
      } else if (_reverbParam == 1) {
        int v = constrain((int)_reverbDecay + step, 0, 100);
        if (v != (int)_reverbDecay) { _reverbDecay = (uint8_t)v; _reverbChanged = true; }
      } else if (_reverbParam == 2) {
        int v = constrain((int)_reverbDamp + step, 0, 100);
        if (v != (int)_reverbDamp) { _reverbDamp = (uint8_t)v; _reverbChanged = true; }
      } else if (_reverbParam == 3) {
        int v = constrain((int)_reverbMix + step, 0, 100);
        if (v != (int)_reverbMix) { _reverbMix = (uint8_t)v; _reverbChanged = true; }
      } else {
        const uint8_t si = _reverbParam - 4;
        int v = constrain((int)_reverbSend[si] + step, 0, 100);
        if (v != (int)_reverbSend[si]) { _reverbSend[si] = (uint8_t)v; _reverbChanged = true; }
      }
      return;
    }
    return;
  ...
  if (_screen == SCR_ECHO) {
    if (in.f[8]) { _echoEditMode = false; go(SCR_EFFECTS, 0); return; }
  ...
  if (_screen == SCR_SAMPLE_EDITOR) {
    ...
    if (in.f[6]) { _loopParam = 0; _loopEditMode = false; go(SCR_LOOP); return; }
...
void PhoenixScreenManager::drawMultisampleEditor(PhoenixGUI &gui) {
  ...
  if (_msLoadStatus[0]) {
    // v1.0.8: make long multisample loads visible instead of looking frozen.
    gui.fill(x + 6, y + 17, w - 12, 31, false);
    gui.frame(x + 8, y + 19, w - 16, 27);
    gui.centered(x + 10, y + 23, w - 20, "LOADING SAMPLE", false);
    gui.centered(x + 10, y + 34, w - 20, _msLoadStatus, false);
    gui.drawClassicHelp("", "PLEASE WAIT", "");
    return;
  }
...
void PhoenixScreenManager::drawReverb(PhoenixGUI &gui) {
  gui.drawClassicLevelMeter(0); const int x=PhoenixLayout::WIN_X,y=PhoenixLayout::WIN_Y,w=PhoenixLayout::WIN_W,h=PhoenixLayout::WIN_H;
  gui.drawWindow(x,y,w,h,"REVERB"); if(_reverbEditMode)gui.text(x+w-22,y+2,"EDIT",false);
  const char* labels[8]={"SIZE","DECAY","DAMP","MIX","SEND S1","SEND S2","SEND S3","SEND S4"}; char value[12];
  // v1.1.0 DEV2b: use the exact same 8-row geometry as the Phoenix main menu.
  // The previous 5-pixel row pitch was too tight for the OLED font and caused overlap.
  const int firstY = y + PhoenixLayout::MENU_FIRST_Y;
  const int rowH   = PhoenixLayout::MENU_ROW_H;
  for(uint8_t i=0;i<8;++i){int yy=firstY+i*rowH;uint8_t v=(i==0)?_reverbSize:(i==1)?_reverbDecay:(i==2)?_reverbDamp:(i==3)?_reverbMix:_reverbSend[i-4];snprintf(value,sizeof(value),"%u%%",(unsigned)v);gui.text(x+6,yy,(i==_reverbParam)?">":" ",false);gui.text(x+11,yy,labels[i],false);gui.text(x+78,yy,value,false);}
  gui.drawClassicHelp(_reverbEditMode?"TURN VALUE":"TURN SELECT",_reverbEditMode?"PUSH DONE":"PUSH EDIT","8BACK");
}
...
void PhoenixScreenManager::drawLoop(PhoenixGUI &gui) {
  ...
  if (_loopEditMode) gui.text(x+w-22,y+2,"EDIT",false);
  const uint8_t sl=_selectedSlot&3U;
  ...
  if (_loopEditMode) gui.drawClassicHelp("TURN VALUE","PUSH DONE 7PLAY","8BACK");
  else gui.drawClassicHelp("TURN SELECT","PUSH EDIT 7PLAY","8BACK");
}
...
void PhoenixScreenManager::drawDiskUtilities(PhoenixGUI &gui) {
  ...
  if (_diskBankSelectAction < 0) {
    ...
    gui.drawClassicHelp("TURN SELECT", "PUSH OPEN", "8BACK");
  } else {
    const char *op = (_diskBankSelectAction == 1) ? "LOAD BANK" :
                     ((_diskBankSelectAction == 0) ? "SAVE BANK" : "DELETE BANK");
    char bankText[20];
    snprintf(bankText, sizeof(bankText), "BANK %02u", (unsigned)_diskBank);
    gui.text(x + 8, y + 15, op, false);
    gui.text(x + 8, y + 27, bankText, false);
    gui.text(x + 8, y + 37, _diskBankUsed ? "USED" : "EMPTY", false);
    gui.fill(x + 4, y + 47, w - 8, 7, false);
    gui.text(x + 6, y + 47, _diskStatus, false);
    gui.drawClassicHelp("TURN BANK", "PUSH DO", "8BACK");
  }
}
...
void PhoenixScreenManager::draw(PhoenixGUI &gui) {
  ...
  switch (_screen) {
    ...
    case SCR_EFFECTS: { const char *items[2]={"ECHO","REVERB"}; gui.drawClassicMenu("EFFECTS",items,2,_sel,true); gui.drawClassicHelp("1UP","3DOWN","7SELECT 8BACK"); break; }
