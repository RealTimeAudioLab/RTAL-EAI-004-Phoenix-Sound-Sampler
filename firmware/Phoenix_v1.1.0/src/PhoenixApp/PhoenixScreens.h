...
public:
  ...
  // v1.0.12: loaded/active bank is distinct from the Disk Utilities cursor.
  uint8_t selectedBank() const { return _activeBank; }
  uint8_t activeBank() const { return _activeBank; }
  uint8_t diskBank() const { return _diskBank; }
  ...
  void setSelectedBank(uint8_t bank) {
    bank = constrain(bank, (uint8_t)1, (uint8_t)99);
    _activeBank = bank;
    _diskBank = bank;
  }
  ...
  void setMultisampleLoadStatus(const char *text);
  void invalidateSampleBrowserCache();
  ...
  void setReverbStatus(uint8_t size, uint8_t decay, uint8_t damp, uint8_t mix, const uint8_t send[4]);
  ...
  uint8_t reverbSend(uint8_t slot) const { return _reverbSend[slot & 3U]; }
...
private:
  ...
  bool _envChanged;
  uint8_t _echoParam;
  uint8_t _reverbParam;
  uint8_t _reverbSize, _reverbDecay, _reverbDamp, _reverbMix;
  uint8_t _reverbSend[4];
  ...
  bool _loopEditMode; // false=navigate parameters, true=edit selected value
  ...
  uint8_t _diskBank;
  bool _diskBankUsed;
  int8_t _diskBankSelectAction; // -1=menu, 0=save, 1=load, 2=delete
  bool _bankDirty;
  ...
  struct BrowserEntry {
    ...
    uint32_t size;
    bool wavInfoValid;
    bool wavInfoScanned; // v1.0.6: metadata is loaded lazily for selected WAV only
    uint32_t sampleRate;
  ...
  uint8_t _browserSel;
  bool _browserCacheValid; // v1.0.6: current-directory RAM cache can be reused on re-entry
  ...
  void drawReverb(PhoenixGUI &gui);
  ...
  void ensureSelectedBrowserWavInfo();
  void browserGoRoot();
  bool browserGoUp();
