...
void PhoenixOS::initializeNewBank() {
  ...
  _msConfigDirty=false;
  _newBankPendingSave=true;
  _screens.setBankDirty(true);_screens.setDiskStatus(templateLoaded?"NEW BANK: INIT CFG":"NEW BANK: FACTORY");
...
void PhoenixOS::update() {
  ...
  if (diskAction >= 0) {
    ...
    if (!phxEnsureSD()) {
      ...
      const bool allowIntentionalEmptyBank = _newBankPendingSave;
      bool ok = _audio.saveBankToSD(bankDir, allowIntentionalEmptyBank);
