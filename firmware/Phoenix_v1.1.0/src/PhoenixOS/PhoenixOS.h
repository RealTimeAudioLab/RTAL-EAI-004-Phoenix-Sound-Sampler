...
private:
  ...
  bool _bootSessionRestoreActive;
  uint8_t _bootSessionBank;
  uint32_t _lastSessionSaveMs;
  uint8_t _sessionBankCache;
  uint8_t _sessionScreenCache;
  uint8_t _sessionSlotCache;
  uint8_t _sessionPendingBank;
  uint8_t _sessionPendingScreen;
  uint8_t _sessionPendingSlot;
  bool _sessionDirty;
  uint32_t _sessionLastChangeMs;
  bool _midiEchoSync;
  ...
  // True only after the user explicitly executed NEW BANK. It authorizes one
  // metadata-only SAVE BANK while all WAV/multisample payloads are empty.
  bool _newBankPendingSave;

  // v1.0.9: Multisample editor changes are applied to RAM immediately, but
  // MULTISAMPLE.CFG is saved only after an edit quiet-time and never while
  // the sequencer/song transport is running. This keeps SD/FAT I/O out of
  // realtime parameter editing.
  bool _msConfigDirty;
  uint8_t _msConfigDirtyBank;
  uint32_t _msConfigLastEditMs;
  uint32_t _msConfigDeferredSaveCount;
  void markMultisampleConfigDirty();
  void processDeferredMultisampleConfigSave();
  bool saveDirtyMultisampleConfig();
  ...
  void initializeNewBank();
  void syncNewBankStateToScreens();
  void quiesceForBankIo(const char *reason);
  bool loadBankForSession(uint8_t bank);
  void restoreLastSession();
  void saveSessionIfChanged();
  void saveSessionNow(); // v1.0.11 persist authoritative bank/view immediately
