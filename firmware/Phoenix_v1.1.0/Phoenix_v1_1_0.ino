// RTAL Project Phoenix v1.1.0 FINAL
// Release basis: hardware-validated MULTISAMPLE BUFFERED LOAD PERF1.
// SD SPI 10 MHz, LOAD_PROGRESS_FIX1 and NEWBANK_EMPTY_SAVE_FIX1 integrated.
// Buffered multisample WAV loading integrated; Audio/DSP/BANK16/session behavior unchanged.
// Serial output disabled for the release build.
// Session bank/screen/slot does not use NVS; foreign NVS data remains untouched.
// Reverb delay memory uses internal SRAM with safe fallback; Echo remains in PSRAM.
// Diagnostic Reverb cycle timing is compiled out by default.
// Retains validated Core0 WDT yields and BANK-SAVE-PERF2 transaction behavior.
/*
...
#include "src/PhoenixSystem/PhoenixSerial.h"
#define OLED_SCK   12
#define OLED_MOSI  11
#define OLED_CS    10
#define OLED_DC     6
...
static void controlTask(void *param) {
  (void)param;
  PHX_INFO_PRINTF("V1.0 CONTROL task core=%d priority=%u\n", xPortGetCoreID(), (unsigned)uxTaskPriorityGet(nullptr));
  for (;;) {
    phoenix.update();
    vTaskDelay(pdMS_TO_TICKS(1));
  }
...
void setup() {
  ...
  // v1.0.12a: robust service-mode detection. A single transient LOW at boot
  // must never suppress last-session restore. Require the encoder button to
  // remain pressed for the complete ~120 ms sampling window.
  delay(20);
  bool serviceMode = true;
  for (uint8_t i = 0; i < 8; ++i) {
    if (digitalRead(ENC_SW_PIN) != LOW) serviceMode = false;
    delay(15);
  }
#if PHX_SERIAL_LEVEL >= 2
  ...
  PHX_INFO_PRINTF("V1.0 SETUP core=%d; Audio=Core1/P24, MIDI=Core0/P10, SeqRT=Core0/P8, Control=Core0/P3\n", xPortGetCoreID());
}
